...
#if defined(NDEBUG)
# error "FC cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "FC Signed Message:\n";
                        ...
                        bool* pfMissingInputs)
{
    ...
    // FC: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// FC: find block wanted by given orphan block
...
// FC: find last block index up to pindex
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // FC: target change every block
    // FC: retarget with exponential moving toward target spacing
    CBigNum bnNew;
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    // fMiner is true when called from the internal FC miner
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // FC: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // FC: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // FC: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // FC: track money supply and mint amount info
...
// FC: total coin age spent in transaction, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    // FC: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // FC: compute stake entropy bit for stake modifier
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom) {
            ...
            // FC: getblocks may not obtain the ancestor block rejected
...
#ifdef ENABLE_WALLET
// FC: attempt to generate suitable proof-of-stake
